...
Sprite3D::Sprite3D(GameObject* pGameObject):
...
void Sprite3D::Draw()
{
	...
	VertexPositionColorTexture p;
	vertices[0] = VertexPositionColorTexture(Vector3(m_size.x, m_size.y, 0) * m_scale, Colors::White, Vector2(m_rect.left, m_rect.top));
	vertices[1] = VertexPositionColorTexture(Vector3(-m_size.x, m_size.y, 0) * m_scale, Colors::White, Vector2(m_rect.right, m_rect.top));
	vertices[2] = VertexPositionColorTexture(Vector3(-m_size.x, -m_size.y, 0) * m_scale, Colors::White, Vector2(m_rect.right, m_rect.bottom));
	vertices[3] = VertexPositionColorTexture(Vector3(m_size.x, -m_size.y, 0) * m_scale, Colors::White, Vector2(m_rect.left, m_rect.bottom));

	// 変換する
	Transform* pParent = m_pTransform->GetParent();
	Vector3 pos = pParent->GetPosition() + m_pTransform->GetPosition();

	// ワールド行列を作成する
	Matrix world = m_pTransform->GetWorld() * m_pTransform->GetParent()->GetWorld();
