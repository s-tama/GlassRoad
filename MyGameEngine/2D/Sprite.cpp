...
Sprite::Sprite(GameObject* pGameObject):
...
/// 描画処理
/// </summary>
void Sprite::Draw(DirectX::SpriteBatch* pSpriteBatch)
{
	if (m_pTexture)
	{
		...
		Vector2 pos = m_pTransform->GetPosition();
		if (m_pTransform->GetParent() != nullptr)
		{
			pos = static_cast<Vector2>(pParent->GetPosition() + m_pTransform->GetPosition());
		}
		pSpriteBatch->Draw(m_pTexture->pHandle.Get(), pos - offset, &m_rect, m_color, m_rotation, Vector2::Zero, Vector2(m_scale, m_scale));
	}
}
