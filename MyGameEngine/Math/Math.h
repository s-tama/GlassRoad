...
namespace MyLibrary
{
	...
	public:
		...
		/// 壁ずりベクトルを求める
		/// </summary>
		/// <param name="front">進行方向のベクトル</param>
		/// <param name="normal">衝突点での法線ベクトル</param>
		/// <returns>壁と平行のベクトル</returns>
		static DirectX::SimpleMath::Vector3 WallScratchVector(DirectX::SimpleMath::Vector3 front, DirectX::SimpleMath::Vector3 normal)
		{
			float a = -front.Dot(Normalize(normal));
			DirectX::SimpleMath::Vector3 w = front + a * Normalize(normal);
			return w;
		}
		...
		/// <returns></returns>
		static DirectX::SimpleMath::Vector3 ReflectVector(DirectX::SimpleMath::Vector3 front, DirectX::SimpleMath::Vector3 normal)
		{
			float a = -front.Dot(Normalize(normal));
			DirectX::SimpleMath::Vector3 w = front + 2 * a * Normalize(normal);
			return w;
		}
		...
		static DirectX::SimpleMath::Vector3 Normalize(DirectX::SimpleMath::Vector3 v)
		{
			// ベクトルの長さ
